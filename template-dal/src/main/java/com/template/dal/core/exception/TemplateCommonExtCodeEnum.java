...
public enum TemplateCommonExtCodeEnum {
    SUCCESS("00", "处理成功"),
    ...
    private static String getDescription(String errorCode) {
        TemplateCommonExtCodeEnum[] var1 = values();
        int var2 = var1.length;

        for (int var3 = 0; var3 < var2; ++var3) {
            TemplateCommonExtCodeEnum item = var1[var3];
            if (StringUtils.equals(item.getCode(), errorCode)) {
    ...
    private TemplateCommonExtCodeEnum(String code, String message) {
        this.code = code;
        this.message = message;
    }

    public TemplateException getException() {
        return new TemplateException(this.getCode(), this.getMessage());
    }
